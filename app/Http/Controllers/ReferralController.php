...
class ReferralController extends Controller
{
    ...
    public function facturationData(Request $request)
    {
        ...
        if (empty($request->input('search_string'))) {
            $referrals = Referral::where('status', 0)
                ->offset($start)
                ->limit($limit)
                ->orderBy('id', 'desc')
                ->get();

            $totalFiltered = Referral::where('status', 0)
                ->count();
        } else {
    ...
    public function referralData(Request $request)
    {
        ...
        if (empty($request->input('search_string'))) {
            if (Auth::user()->role_id > 2 && config('staff_access') == 'own' && $status_id != 2) {
                $facture = DB::table('withdraw_referrals')
                    ->select('facture_no')
                    ->where([
                        ['referral_id', Auth::id()],
                        ['status', $status_id]
        ...
        // if(Auth::user()->role_id > 2 && config('staff_access') == 'own' && $status_id != 2)
        //     $totalData = WithdrawReferral::where([
        //                                 ['referral_id', Auth::id()],
        //                                 ['status', $status_id],
        //                             ])
        //                             ->distinct()
        ...
        // elseif(Auth::user()->role_id > 2 && config('staff_access') == 'own')
        //     $totalData = WithdrawReferral::where('referral_id', Auth::id())
        //                             ->distinct()
        //                             ->count();
        // elseif($status_id != 2)
        //     $totalData = WithdrawReferral::where('status', $status_id)
        //                             ->distinct()
        //                             ->count();
        // else
        //     $totalData = WithdrawReferral::distinct()->count();
        ...
        // if(empty($request->input('search_string'))) {
        //     if(Auth::user()->role_id > 2 && config('staff_access') == 'own' && $status_id != 2) {
        //         $factures = WithdrawReferral::where([
        //                                     ['referral_id', Auth::id()],
        //                                     ['status', $status_id],
        //                                 ])
        //                                 ->offset($start)
        ...
        //                                 ->distinct()
        //                                 ->get();
        //         $totalFiltered = WithdrawReferral::where([
        //                                     ['referral_id', Auth::id()],
        //                                     ['status', $status_id],
        //                                 ])
        //                                 ->distinct()
        //                                 ->count();
        //     } elseif(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
        //         $factures = WithdrawReferral::where('referral_id', Auth::id())
        //                                 ->offset($start)
        //                                 ->limit($limit)
        //                                 ->orderBy('id', 'desc')
        //                                 ->distinct()
        //                                 ->get();
        //         $totalFiltered = WithdrawReferral::where('referral_id', Auth::id())
        //                                 ->distinct()
        //                                 ->count();
        //     } elseif($status_id != 2) {
        //         $factures = WithdrawReferral::where('status', $status_id)
        //                                 ->offset($start)
        //                                 ->limit($limit)
        //                                 ->orderBy('id', 'desc')
        //                                 ->distinct()
        //                                 ->get();
        //         $totalFiltered = WithdrawReferral::where('status', $status_id)
        //                                 ->distinct()
        //                                 ->count();
        //     } else {
        //         $factures = WithdrawReferral::offset($start)
        //                                 ->limit($limit)
        //                                 ->orderBy('id', 'desc')
        //                                 ->distinct()
        //                                 ->get();
        //         $totalFiltered = WithdrawReferral::distinct()
        //                                 ->count();
        //     }
        // }
        // else
        ...
        //     else
        //     {
        //         $factures = WithdrawReferral::where('facture_no', 'LIKE', "%{$search}%")
        //                                 ->offset($start)
        //                                 ->limit($limit)
        //                                 ->orderBy('id', 'desc')
        //                                 ->distinct()
        ...
        //         $totalFiltered = WithdrawReferral::where('facture_no', 'LIKE', "%{$search}%")
        //                                 ->distinct()
        //                                 ->count();
        //     }
        // }
        ...
        if (!empty($factures)) {
            ...
            foreach ($factures as $key => $facture) {
                ...
                foreach ($lims_facture_data as $key => $w_facture) {
                    // $nestedData['date'] = date(config('date_format'), strtotime($w_facture->created_at->toDateString()));
                    // if ($w_facture->status == 1){
